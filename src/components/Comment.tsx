import { Comment, CommentVotes } from "@prisma/client";
...
const Comment = ({
  ...
  const createCommentVote = trpc.useMutation(["comment.create-comment-vote"], {
    onSuccess: () => getUserCommentVotes.refetch(),
  });

  return (
    <div className="flex border-2 border-zinc-500  w-full rounded-2xl">
        ...
        <div>
          ...
          >
            <svg
              ...
              strokeWidth={2}
              stroke={`${
                getUserCommentVotes.data.some(
                  (commentVote: any) =>
                    commentVote.IsLike === true && commentVote.commentId === comment.id,
                )
                  ? "green"
                  : "gray"
              }`}
          ...
          >
            <svg
              ...
              stroke={`${
                getUserCommentVotes.data.some(
                  (commentVote: any) =>
                    commentVote.IsLike === false && commentVote.commentId === comment.id,
                )
                  ? "red"
                  : "gray"
              }`}
